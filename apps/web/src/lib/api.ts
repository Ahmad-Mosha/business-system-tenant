...
export interface BusinessProfitReport {
  sales: string;
  shipping: string;
...
export const getBusinessProfit = (from: string, to: string, bucket: 'day' | 'week' | 'month') =>
  get<BusinessProfitReport>(`/finance/business-profit?from=${from}&to=${to}&bucket=${bucket}`);
